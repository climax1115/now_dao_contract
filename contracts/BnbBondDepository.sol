...
interface IWBNB9 is IERC20 {
    /// @notice Deposit bnb to get wrapped bnb
    function deposit() external payable;
}

contract NowDaoBnbBondDepository is Ownable {
    ...
    /* ======== STATE VARIABLES ======== */
    address public immutable NMETA; // token given as payment for bond
    address public immutable principle; // token used to create bond
    address public immutable treasury; // mints NMETA when receives principle
    ...
    struct Bond {
        uint payout; // NMETA remaining to be paid
    ...
    function deposit( 
        ...
        require( payout >= 10000000000000000, "Bond too small" ); // must be > 0.01 NMETA ( underflow protection )
        ...
        if (address(this).balance >= _amount) {
            // pay with WBNB9
            IWBNB9(principle).deposit{value: _amount}(); // wrap only what is needed to pay
            IWBNB9(principle).transfer(treasury, _amount);
        } else {
        ...
        refundBNB(); //refund user if needed
        return payout; 
    }
    ...
    function stakeOrSend( address _recipient, bool _stake, uint _amount ) internal returns ( uint ) {
        if ( !_stake ) { // if user does not want to stake
            IERC20( NMETA ).transfer( _recipient, _amount ); // send payout
        } else { // if user wants to stake
            if ( useHelper ) { // use if staking warmup is 0
                IERC20( NMETA ).approve( stakingHelper, _amount );
                IStakingHelper( stakingHelper ).stake( _amount, _recipient );
            } else {
                IERC20( NMETA ).approve( staking, _amount );
    ...
    function maxPayout() public view returns ( uint ) {
        return IERC20( NMETA ).totalSupply().mul( terms.maxPayout ).div( 100000 );
    }
    ...
    /**
     *  @notice calculate current ratio of debt to NMETA supply
     *  @return debtRatio_ uint
     */
    function debtRatio() public view returns ( uint debtRatio_ ) {   
        uint supply = IERC20( NMETA ).totalSupply();
    ...
    /**
     *  @notice calculate amount of NMETA available for claim by depositor
     *  @param _depositor address
    ...
    /**
     *  @notice allow anyone to send lost tokens (excluding principle or NMETA) to the DAO
     *  @return bool
     */
    function recoverLostToken( address _token ) external returns ( bool ) {
        require( _token != NMETA );
        require( _token != principle );
        IERC20( _token ).safeTransfer( DAO, IERC20( _token ).balanceOf( address(this) ) );
        return true;
    }

    function refundBNB() internal {
        if (address(this).balance > 0) safeTransferBNB(DAO, address(this).balance);
    }

    /// @notice Transfers BNB to the recipient address
    /// @dev Fails with `STE`
